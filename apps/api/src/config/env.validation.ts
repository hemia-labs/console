...
export const envVarsSchema = Joi.object({
  ...
  SSO_JWKS_URL: Joi.string().uri({ scheme: ['http', 'https'] }).optional(),
  SSO_AUTHORIZATION_URL: Joi.string().uri({ scheme: ['http', 'https'] }).optional(),
  SSO_TOKEN_URL: Joi.string().uri({ scheme: ['http', 'https'] }).optional(),
  SSO_REVOCATION_URL: Joi.string().uri({ scheme: ['http', 'https'] }).optional(),
  SSO_LOGOUT_URL: Joi.string().uri({ scheme: ['http', 'https'] }).optional(),
  ...
  SSO_FRONTEND_URL: Joi.string()
    .uri({ scheme: ['http', 'https'] })
    .default('http://localhost:3000'),
  SSO_SCOPE: Joi.string().default('openid profile email offline_access'),
